...
@property(nonatomic,strong)UICollectionView *collectinoView;
...
//    [self.view addSubview:self.tableView];
    [self.view addSubview:self.collectinoView];
...
-(UICollectionView *)collectinoView
{
    if (!_collectinoView) {
        
        UICollectionViewFlowLayout *layout = [[UICollectionViewFlowLayout alloc]init];
        layout.itemSize = CGSizeMake(self.view.frame.size.width, self.view.bounds.size.height - self.weekView.bounds.size.height-64);
        layout.minimumLineSpacing = 0;
        layout.minimumInteritemSpacing = 0;
        layout.scrollDirection = UICollectionViewScrollDirectionHorizontal;
        _collectinoView= [[UICollectionView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.weekView.frame),self.view.bounds.size.width, self.view.bounds.size.height - self.weekView.bounds.size.height-64) collectionViewLayout:layout];
        _collectinoView.backgroundColor = [UIColor whiteColor];
//        _collectinoView.tag = CONTENT;
        _collectinoView.bounces = NO;
        [_collectinoView registerClass:[MonthTableViewCell class] forCellWithReuseIdentifier:@"cell"];
        _collectinoView.showsHorizontalScrollIndicator = NO;
        _collectinoView.pagingEnabled = YES;
        _collectinoView.delegate = self;
        _collectinoView.dataSource = self;
    }
    return _collectinoView;
}
#pragma mark - 创建星期视图
-(UIView *)weekView{
    if (!_weekView) {
...
-(void)selectedCheckDate{
    ...
    if (isHaveStartDate && isHaveEndDate) {
        ...
        NSString *daysStr = [NSString stringWithFormat:@"%ld",(long)days];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    MonthTableViewCell *cell=[collectionView dequeueReusableCellWithReuseIdentifier:@"cell" forIndexPath:indexPath];
    [cell setContentView];
//    MonthTableViewCell *cell = [MonthTableViewCell cellWithTableView:collectionView];
//    MonthTableViewCell *cell = [MonthTableViewCell cellWithTableView:tableView];
    ...
    cell.selectedDay = ^(DayModel *returnDaymodel){
        ...
        [weakSelf.collectinoView reloadData];
